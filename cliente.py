...
async def _cargar_relaciones(cli: Cliente, db: AsyncSession):
    if cli.cve_ent:
        ent = await db.scalar(select(Entidad).where(Entidad.cvegeo == 
cli.cve_ent))
        if ent:
            cli.entidad = ent
    if cli.cve_ent and cli.cve_mun:
        muni = await db.scalar(select(Municipio).where(and_(
    ...
    if cli.cve_ent and cli.cve_mun and cli.cve_loc:
        loc = await db.scalar(select(Localidad).where(and_(
...
async def listar_clientes(nombre: Optional[str] = Query(None), skip: int = 
0, limit: int = 100, db: AsyncSession = Depends(get_async_db)):
...
db.execute(stmt.offset(skip).limit(limit))).scalars().all()
    ...
    return {"success": True, "total_count": total, "data": 
[ClienteRead.model_validate(c) for c in items]}

@router.get("/{id_cliente}", response_model=ClienteRead)
async def obtener_cliente(id_cliente: UUID, db: AsyncSession = 
...
async def crear_cliente(datos: ClienteCreate, db: AsyncSession = 
Depends(get_async_db)):
    ctx = await obtener_contexto(db)

    # Validar localidad si se dan claves INEGI
    if datos.cve_ent and datos.cve_mun and datos.cve_loc:
        existe = await db.scalar(select(Localidad).where(and_(
...
async def actualizar_cliente(id_cliente: UUID, cambios: ClienteUpdate, db: 
...
async def borrar_cliente(id_cliente: UUID, db: AsyncSession = 
Depends(get_async_db)):
    estado_activo = await get_estado_id_por_clave("act", db)
    cli = await db.scalar(select(Cliente).where(Cliente.id_cliente == 
id_cliente, Cliente.id_estado == estado_activo))
